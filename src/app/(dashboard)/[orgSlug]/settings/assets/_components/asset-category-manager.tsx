...
import { createAssetCategory, updateAssetCategory } from '@/modules/assets/actions'
...
export function AssetCategoryManager({ orgSlug, categories, canManage }: AssetCategoryManagerProps) {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    const result = await createAssetCategory(orgSlug, { name: createName })
  ...
  const handleEdit = async (e: React.FormEvent) => {
    ...
    const result = await updateAssetCategory(orgSlug, {
  ...
  const handleArchiveToggle = async (categoryId: string, isArchived: boolean) => {
    await updateAssetCategory(orgSlug, { categoryId, isArchived: !isArchived })
    router.refresh()
  }
    ...
    <div className="space-y-4">
      ...
      {activeCategories.length === 0 && archivedCategories.length === 0 ? (
        <EmptyState
          ...
          description="Create a category to start organising company assets."
        />
      ) : (
        <Card>
      ...
      <Dialog open={showCreate} onOpenChange={(v) => { if (!v) { resetCreate(); setShowCreate(false) } }}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Create a new category for company assets.</DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={!!editingId} onOpenChange={(v) => { if (!v) resetEdit() }}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Update the asset category name.</DialogDescription>
          </DialogHeader>
