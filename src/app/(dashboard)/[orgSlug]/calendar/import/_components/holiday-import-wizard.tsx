...
import { validateHolidayImportCsv, commitHolidayImportCsv } from '@/modules/calendar/import-actions'
import { CSV_HEADERS } from '@/modules/calendar/import-constants'
...
export function HolidayImportWizard({ orgSlug }: HolidayImportWizardProps) {
  ...
  function handleValidate() {
    ...
    startTransition(async () => {
      const res = await validateHolidayImportCsv(orgSlug, csvText)
      if (!res.success) {
  ...
  function handleCommit() {
    startTransition(async () => {
      const res = await commitHolidayImportCsv(orgSlug, csvText)
      if (!res.success) {
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-lg border border-border bg-surface p-4 space-y-2">
          ...
          <ul className="text-[12px] text-text-muted space-y-1 list-disc list-inside">
            <li>CSV file with headers: <strong>date</strong>, <strong>name</strong></li>
            <li><strong>date</strong> format: YYYY-MM-DD</li>
            <li><strong>name</strong>: holiday name (required)</li>
            <li>Duplicate holidays (same date + name) will be flagged as errors</li>
              ...
              <tbody>
                {rows.map((row) => (
                  ...
                  >
                    ...
                    <td className="px-2 py-1.5 text-text">{row.date || '—'}</td>
        ...
        <div className="flex items-center gap-3">
          ...
          <Button onClick={handleCommit} disabled={isPending || validCount === 0}>
            {isPending ? (
              ...
              <>
                Import {validCount} Holiday{validCount !== 1 ? 's' : ''}
                <ArrowRight className="h-4 w-4 ml-2" />
              </>
            )}
          </Button>
      ...
      <div className="rounded-lg border border-border bg-surface p-6 space-y-4">
        ...
        <div className="space-y-2">
          <p className="text-[13px] text-text">
            <span className="text-success font-medium">{result?.created}</span> holiday{result?.created !== 1 ? 's' : ''} imported successfully.
          </p>
          {(result?.failed ?? 0) > 0 && (
                ...
                <tbody>
                  {result.failures.map((f) => (
                    <tr key={f.rowIndex} className="border-b border-border last:border-0">
                      ...
                      <td className="px-2 py-1.5 text-text">{f.date}</td>
